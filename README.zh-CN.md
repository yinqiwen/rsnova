...
- 当前版本的 client/server 必须同步升级。线协议新增目标连接确认与 tunnel 排空控制，不兼容旧版本。
- SOCKS5 与 HTTP `CONNECT` 仅在服务端实际连接目标成功后返回成功；目标不可达会明确反馈给本地客户端，不再表现为成功后的静默断开。
- `--connection-max-age` 采用优雅退役：停止分配新 stream，等待单向或双向传输结束，再关闭并替换连接。
- 反向隧道达到最大连接年龄或配置热更新时，会先注册替代代，再排空旧代，已有反向流不中断且路由无空窗。
